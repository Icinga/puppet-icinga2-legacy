...
class icinga2::params {
  ...
  case $operatingsystem {
    ...
    'RedHat', 'CentOS'  : {
      ...
      $icinga2_server_plugin_packages = [
        "nagios-plugins-nrpe",
        "nagios-plugins-all",
  ...
  case $operatingsystem {
    ...
    'RedHat', 'CentOS'  : { $icinga2_server_service_name = 'icinga2' }

    # Icinga 2 server daemon names for Debian/Ubuntu systems:
    /^(Debian|Ubuntu)$/ : { $icinga2_server_service_name = 'icinga2' }
  ...
  case $operatingsystem {
    ...
    'RedHat', 'CentOS'  : { $nrpe_daemon_name = 'nrpe' }

    # Daemon names for Debian/Ubuntu systems:
    /^(Debian|Ubuntu)$/ : { $nrpe_daemon_name = 'nagios-nrpe-server' }
